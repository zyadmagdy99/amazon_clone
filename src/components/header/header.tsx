...
interface stateProps {
  next: {
    ...
    favouriteData: productProps[];
    userInfo: { name: string; email: string; image: string } | null;
    allProducts: { allProduct: productProps[] };
  };
}
...
const Header: React.FC = () => {
  ...
  const [filterData, setFilterData] = useState<productProps[]>([]);
  ...
  useEffect(() => {
    const filtered = allData.filter((item: storeProduct) =>
      item.title.toLocaleLowerCase().includes(inSearch.toLowerCase())
    );
  ...
  return (
    <div className="w-full h-20 sticky bg-amazon_blue text-lightText top-0 z-50">
      <div className="h-full w-full relative mx-auto justify-between gap-1 px-4 inline-flex items-center">
        ...
        <Link
          href={"/"}
          className="cursor-pointer border border-transparent hover:border-white px-2 duration-300 h-[70%] flex justify-center items-center"
        >
        ...
        <div className="flex-1 h-10 absolute md:top-0 md:left-0 min-w-[20rem] top-20 left-20 md:inline-flex items-center justify-between md:relative">
          ...
          {inSearch && (
            <div className="absolute min-w-[30rem] -left-20 md:left-0 top-12 w-full mx-auto max-h-96 bg-gray-200 rounded-lg overflow-y-scroll cursor-pointer text-black">
              {filterData.length > 0 ? (
                <>
                  {filterData.map((item: storeProduct) => (
                    <Link
                      onClick={() => {
                        setInSearch("");
                      }}
                ...
                <div className="bg-gray-50 flex items-center justify-center py-10 ps-5 rounded-lg shadow-lg">
        ...
        {userInfo ? (
          ...
          <div
            ...
            className="text-xs text-gray-100 flex flex-col cursor-pointer border border-transparent hover:border-white px-2 duration-300 h-[70%] justify-center"
          >
            <p>Hello, sign in</p>
        ...
        <Link
          href="/favourite"
          className="text-xs relative text-gray-100 flex flex-col cursor-pointer border border-transparent hover:border-white px-2 duration-300 h-[70%] justify-center"
        >
          <p>Marked</p>
        ...
        <Link
          href={"/Cart"}
          className="items-center flex relative cursor-pointer border border-transparent hover:border-white px-2 duration-300 h-[70%]"
        >
